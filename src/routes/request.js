...
requestRouter.post("/user/:status/:toUserId", authorize, async (req, res) => {
  try {
    ...
    if (!validStatuses.includes(status)) {
      return res
        .status(400)
        .json({ message: "Invalid status --/user/:status/:toUserId" });
    }
    ...
    if (existingConnection) {
      return res.status(400).json({
        message: "Connection already exists --/user/:status/:toUserId",
      });
    }
    ...
    if (!toUser) {
      return res
        .status(400)
        .json({ message: "User does not exist --/user/:status/:toUserId" });
    }
  ...
  async (req, res) => {
    try {
      ...
      if (!validStatuses.includes(status)) {
        return res
          .status(400)
          .json({ message: "Invalid status---/review/:status/:requestId" });
      }
      ...
      if (!connectionRequest) {
        return res
          .status(400)
          .send("No Request found ---/review/:status/:requestId");
      }
      ...
      res
        .status(400)
        .send("ERROR in /review/:status/:toUserId---" + err.message);
    }
  }
);
...
requestRouter.get("/user/request/received", authorize, async (req, res) => {
  try {
    ...
    const receivedRequests = await ConnectionRequest.find({
      ...
      .populate("fromUserId", ["firstName", "lastName", "email", "imageUrl"])
      .lean();
